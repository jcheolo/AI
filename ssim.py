# -*- coding: utf-8 -*-
"""
Created on Sat Aug 26 14:56:39 2023
...
class SSIM(nn.Module):
    ...
    def _ssim(self, x, y):
        ...
        #l = (2*torch.sqrt(A1*B1) + 0.001)/(A1 + B1 + 0.001)
        #cs = (2*torch.sqrt(A2*B2) + 0.001)/(A2 + B2 + 0.001)
        #ssim = (2*torch.sqrt(l*cs)+0.001/(l+cs+0.001 )

        
        
        #gaussian_L1 = self.gaussian_filter(x-y)
        #M = self.gaussian_filter(torch.ones_like(x))
        #gaussian_L1 = abs(gaussian_L1)
        
        #ssim = (1-ssim.mean())*0 + gaussian_L1.mean()/M.mean()
